...
class GetTopRatedMoviesUseCaseTest{
    ...
    @Before
    fun setUp() {
        getTopRatedMoviesUseCase = GetTopRatedMoviesUseCase(moviesRepo)
    }

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun testGetNowPlayingMoviesList() = testCoroutineRule.runTest{
        whenever(moviesRepo.getTopRatedMovies(0)).thenAnswer{
        ...
        val flow = getTopRatedMoviesUseCase(0).first()
